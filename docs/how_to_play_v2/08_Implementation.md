...
* `SecondGameTutorialScreen`（残響体・復讐の鬼影、各3ステップ）
* チュートリアルピッカーからの手動起動
* **初回脱落時**の案内ダイアログ（`offerSecondGameTutorialIfNeeded`）
* 試合中 `EliminationSupportBar` の **「操作を練習」** 導線
* チュートリアルピッカー下部の **作戦マニュアルを見る**

## 完了（パフォーマンス・第1〜2段階）

* マップオーバーレイの `ValueNotifier` 分離（`_publishMapOverlay`）
* `_renderPump` の全画面 `setState` 廃止・移動閾値 0.75m
* 作戦マニュアルの `SliverChildBuilderDelegate` 遅延構築
* 試合中 GPS 更新時の `setState` 抑制（オーバーレイのみ更新）

## 完了（専用 Screen）

* `HowToPlayScreen`（全画面作戦マニュアル）
* ガイドハブ → 作戦マニュアルは全画面が既定。シート表示も選択可
* 試合中は従来どおり `showHowToPlaySheet`（ボトムシート）

## 完了（図解追加）

* DIA-INFO-003 情報の強さ / DIA-INFO-004 痕跡をつなぐ
* DIA-ONLINE-001 試合中止と記録
* `HelpMapConceptDiagram` をマップ施設章に接続
* `HelpFactionDiagram` は `FactionWinDiagram` で代替済み（互換ウィジェットは残置）

## 完了（パフォーマンス第3段階・一部）

* マーカー `fingerprint` キャッシュ（`GameMapInteractiveLayer`）
* **低スペックモード**（HUD 表示設定：更新頻度・カメラパルス抑制）

## 一部完了（HUD 文言）

* 試合中コントロールパネル・準備パネルの **「遊び方」→「作戦マニュアル」**
* `MatchUiTerms` に共通ラベル・`learnMoreHint` を集約
* 通知・イベントログの全文統一は `07_CopyWriting.md` 残り

## 未対応（意図的に後回し）

* **HUD・通知・イベントログの全文言統一**（`07_CopyWriting.md` 残り）
* **チュートリアル高度演出**（匿名痕跡タップ・パニック警告・複数痕跡追跡など）
* **チュートリアル開始専用 Screen**（ピッカーにマニュアル直開きは追加済み）
* **図解の未実装分**（情報の強さ・痕跡をつなぐ・拘束分岐フロー等、`06_Diagrams.md` 参照）
* **ポリライン／サークルの diff** などパフォーマンス第4段階
