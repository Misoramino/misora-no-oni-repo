...
abstract final class SkillCatalog {
  static const matchFlow =
      'タイトル → ルーム参加 → 準備（時間・エリア）→ カスタム設定（任意）→ 試合開始 → '
      '結果。推奨: 5〜6人・30〜60分・やや広めのエリア（第二ゲーム・告発・心理戦向け）。';

  static const coreRule =
      '通常は仲間のライブ位置は見えません。位置暴露・情報屋・マップギミック・スキルで断片情報が出ます。'
  ...
  static const runnerSkills = [
    SkillHelpEntry(
      ...
      body:
          ...
          '発動中は名前付きの位置暴露（感染・情報屋の副作用など）が出るとき、本当のGPSではなくデコイ座標に差し替わります。'
    ...
    SkillHelpEntry(
      ...
      body:
          '地図タップで捕獲結界を設置（鬼の接触圏・感染とは別）。範囲内の相手をロックし、至近またはBLEで捕獲しやすくします。'
          '足止め・逆転トラップ向け。半径約55m・約24秒・再使用まで約80秒。',
    ),
  ];
  ...
  static const hunterSkills = [
    ...
    SkillHelpEntry(
      ...
      iconName: 'trip_origin',
      body:
          '地図タップで捕獲結界を設置（鬼の接触圏・感染とは別）。範囲内の逃走者をロックし、至近またはBLEで捕獲しやすくします。'
          '半径約55m・約24秒。範囲外に10秒出ると相手に脱落リスク。再使用まで約80秒。',
    ),
  ];
  ...
  static const gimmicks = [
    ...
    SkillHelpEntry(
      ...
      body:
          '鬼からの距離が「感染環」（プレイエリア連動・接触圏より外側になりやすい）に'
          '約6秒 → 約22秒の感染。約7秒ごとに名前付き暴露。脱落はしません。',
    ),
    SkillHelpEntry(
      id: 'capture',
      title: '接触・捕獲',
    ...
    SkillHelpEntry(
      ...
      body:
          ...
          '（脱落1人かつ5分）または試合60%で解禁。正解で逃走者即勝利。'
          '失敗で告発者即脱落（残響体へ）。人狼は告発不可。'
