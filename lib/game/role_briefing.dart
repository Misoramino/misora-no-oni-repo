...
abstract final class RoleBriefingCatalog {
  ...
  static const _runnerStart = MatchStartBriefing(
    ...
    tagline: '鬼から逃げて、時間を切り抜ける側です。',
    winLine: '制限時間まで生き残れば、あなたたちの勝ちです。',
    mustKnow: [
      '鬼の正確な位置は、ずっと見え続けるわけではありません。',
      '近くに長くいると捕まります。地図の手がかりを頼りに逃げましょう。',
      '人数が多い試合では、あとから施設で本鬼を当てる一発勝ちもあります。',
    ],
  );

  static const _hunterStart = MatchStartBriefing(
    role: PlayerRole.hunter,
    tagline: '逃げる人を追い、捕まえる本鬼です。',
    winLine: '逃走者を捕まえて、鬼側の勝ちを目指します。',
    mustKnow: [
      '相手のリアルタイム位置は見えません。痕跡やカメラで追いましょう。',
      '近くに寄り続けると拘束でき、至近距離で捕獲できます。',
  ...
  static MatchStartBriefing _werewolfStart(FactionSide? faction) {
    final factionLine = switch (faction) {
      FactionSide.humanTeam => 'この試合は人側（${faction!.label}）です。',
      FactionSide.oniTeam => 'この試合は鬼側（${faction!.label}）です。',
      null => 'この試合の味方は、人数で決まります（HUD に表示）。',
    };
    final styleHint = switch (faction) {
      FactionSide.humanTeam =>
        '人側のとき、鬼化すると近づいて仲間を脅かせます（捕まえもできます）。',
      FactionSide.oniTeam =>
        '鬼側のとき、鬼化しても直接は殺せません。追い回して本鬼に追わせるのが基本です。',
      null => '鬼化中は周りから鬼に見えます。陣営によって捕まえられるかが変わります。',
    };
    return MatchStartBriefing(
      ...
      winLine: factionLine,
      mustKnow: [
        'スキルで「人の姿」と「鬼の姿」を切り替えられます。',
        styleHint,
        '細かい切替タイミングは、プレイしながら慣れていきましょう。',
      ],
    );
  }
