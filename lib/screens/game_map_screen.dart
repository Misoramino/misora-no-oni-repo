...
/// | ???? | ?? |
/// |---|---|
/// | [game_map_screen.dart] | ????????init/build?GPS????UI?? |
/// | `game_map_screen.online_sync.dart` | Firestore ????????? |
/// | `game_map_screen.reveals_gimmicks.dart` | ????????????? |
/// | `game_map_screen.hud_experience.dart` | HUD ??????????????????? |
/// | `game_map_screen.play_area.dart` | ??????????? |
/// | `game_map_screen.match_lifecycle.dart` | ????/???????????? |
/// | `game_map_screen.accusation.dart` | ???? |
/// | `game_map_screen.second_game.dart` | ???????/??? |
/// | `game_map_screen.skills.dart` | ????????? |
/// | `game_map_screen.overlay.dart` | ???????????????? |
///
/// ??????? part ??????????????
/// `lib/features/game_map/game_map_screen_index.dart` ????
///
/// `_GameMapScreenState` ? private ??????????????????
...
class _GameMapScreenState extends State<GameMapScreen>
  ...
  Future<void> _simulateOfflineFlush() async {
    ...
    if (mounted && _testMode) {
      _toast('???????: ${okIds.length}/${pending.length} ????');
    }
  }
  ...
  Future<void> _setupProximity() async {
    ...
    _proximitySubscription = _proximityService.watch().listen((signal) {
      ...
      setState(() {
        _proximityText =
            '??: ${signal.band.name} (${(signal.confidence * 100).toStringAsFixed(0)}%)';
  ...
  Future<void> _onCameraIdle() async {
    ...
    try {
      ...
      if (mounted) _publishMapOverlay(force: true);
    } catch (_) {}
  }
  ...
  String? _werewolfHudSummary() {
    ...
    final form = _rt.werewolfInOniForm ? '??' : '?';
    if (_rt.werewolfInOniForm) {
      final cap = _werewolfCanCaptureNow ? '???' : MatchUiTerms.panicOnly;
  ...
  String _werewolfStatusSuffix() {
    ...
    final frozen = _factionAtDeath != null ? '????' : '';
  ...
  void _onMapTap(LatLng pos) {
    ...
    setState(() {
      if (_editCircleMode && _waitingCircleCenterTap) {
        ...
        _statusMessage = '?? ${_polygonDraft.length} ??????????';
      }
    });
  }
  ...
  void _undoLastVertex() {
    ...
    setState(() {
      ...
      _polygonDraftClosed = false;
      _statusMessage = '???1???????${_polygonDraft.length}??';
    });
  }
        ...
        return SizedBox(
          ...
          child: ListView(
            ...
            children: [
              ...
              if (_rt.revealLog.isEmpty) const Text('???????'),
  ...
  Future<void> _backToTitle() async {
    if (!await _confirmLeaveActiveMatch('???????????')) return;
    _matchTimer?.cancel();
    _matchRecorder?.discard();
    _matchRecorder = null;
  ...
  Future<void> _openRoomLobby() async {
    ...
    if (returned != null && returned.roomId != null) {
      ...
      _statusMessage = '??? ${returned.roomId} ?????';
  ...
  Future<bool> _confirmLeaveActiveMatch(String title) async {
    ...
    final ok = await showDialog<bool>(
      ...
      builder: (ctx) => AlertDialog(
        title: Text(title),
        content: Text('$secondGameNote?????????$onlineNote'),
        actions: [
          TextButton(
            onPressed: () => Navigator.pop(ctx, false),
            child: const Text('?????'),
  ...
  Widget build(BuildContext context) {
    ...
    final appTitle = narrow
        ...
        : switch (_gameState) {
            ...
            GameState.running => 'Oni Game ? ????',
            GameState.runnerWin => 'Oni Game ? ????',
            GameState.caughtByOni => locallyEliminated
          ...
          actions: [
            if (!_matchPresentationActive) ...[
              ...
              GameMapOverflowMenu(
                ...
                testMode: _testMode,
                onSelected: (value) =>
                    unawaited(_onOverflowMenuSelected(value)),
              ),
            ],
          ],
        ),
              ...
              Positioned(
                ...
                child: matchControlsActive
                  ...
                  roleLabel: locallyEliminated
                      ? eliminationCopy!.roleTitle
                      : (_isHunterNow ? '?' : _localRole.displayName),
                  matchDurationLabel: _matchDurationLabel(),
                  ...
                  onPrepShowMap: () => _enterPrepMapMode(PrepMapMode.browse),
                    ...
                    : GameControlPanel(
                        ...
                        isEditing: _editingArea,
                        fakeSkillActive: _rt.fakePositionActive,
                        roleLabel: _isHunterNow ? '?' : _localRole.displayName,
                        matchDurationLabel: _matchDurationLabel(),
                        canFakeSkill: false,
